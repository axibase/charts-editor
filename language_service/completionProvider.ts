...
export const snippets = require("./snippets.json");
...
export class CompletionProvider {
  ...
  public constructor(textDocument: TextDocument, position: Position) {
    const text: string = textDocument
      .getText()
  ...
  private completeFor(): CompletionItem {
    const regexp: RegExp = /^[ \t]*(?:list|var)[ \t]+(\S+)[ \t]*=/gm;
...
for \${1:${item}} in \${2:${collection}}
  ...
  private completeIf(): CompletionItem[] {
    const regexp: RegExp = /^[ \t]*for[ \t]+(\w+)[ \t]+in/gim;
    const endFor: RegExp = /^[ \t]*endfor/gim;
    let match: RegExpExecArray | null = regexp.exec(this.text);
    let lastMatch: RegExpExecArray | undefined;
...
\${0}`;
    ...
    return [ifString, ifNumber, ifElseIf].map(
  ...
  private completeSettingValue(settingName: string): CompletionItem[] {
    ...
    switch (setting.type) {
      ...
      case "integer":
        if (setting.example) {
          return [
            this.fillCompletionItem({ insertText: setting.example.toString() })
          ];
        }
        break;
      case "boolean": {
      ...
      case "enum": {
        return this.getItemsArray(
          setting.enum.map(el => el.replace(/percentile\\.+/, "percentile(n)"))
        );
      }
      case "interval": {
  ...
  private completeSnippets(): CompletionItem[] {
    const items: CompletionItem[] = Object.keys(snippets).map((key: string) => {
      ...
      return this.fillCompletionItem({
        insertText,
        detail: snippets[key].description,
        name: key,
      ...
      setting.script.fields.forEach((field: Field) => {
        if (field.type === "function") {
          ...
          if (field.args) {
            ...
            let requiredArgsString: string = `${requiredArgs
              .map(field => field.name)
              .join(", ")}`;
            itemFields.insertText = `${field.name}${
              requiredArgsString !== "" ? "(" + requiredArgsString + ")" : ""
            }`;
  ...
  private fillCompletionItem(itemFields: ItemFields): CompletionItem {
    let item: CompletionItem = CompletionItem.create(
      itemFields.name || itemFields.insertText
    );
    item.insertTextFormat =
      itemFields.insertTextFormat || InsertTextFormat.PlainText;
  ...
  private getItemsArray(
    ...
    let items: CompletionItem[] = processedArray.map(el =>
      this.fillCompletionItem({ insertText: el })
    );
